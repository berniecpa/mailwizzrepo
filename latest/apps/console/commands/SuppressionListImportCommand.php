...
class SuppressionListImportCommand extends ConsoleCommand
{
        ...
        try {
            ...
            if (in_array($extension, ['csv']) && is_file($file)) {
                ...
                if (!empty($list) && ($server = DeliveryServer::pickServer())) {
                    $listOverviewUrl = container()->get(OptionUrl::class)->getCustomerUrl('suppression-lists/' . $list->list_uid . '/emails/index');
    ...
    protected function processCsv(array $params)
    {
        ...
        $importAtOnce     = container()->get(OptionImporter::class)->getImportAtOnce();
        $insert           = [];
        $mainCounter      = 0;
        $count            = 0;
