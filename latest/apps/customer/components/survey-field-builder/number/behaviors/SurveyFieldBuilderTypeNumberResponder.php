...
/**
 * SurveyFieldBuilderTypeNumberResponder
 *
...
/**
 * @property SurveyFieldBuilderTypeNumber $owner
 */
class SurveyFieldBuilderTypeNumberResponder extends SurveyFieldBuilderTypeResponder
{
    /**
     * @param CEvent $event
     *
    ...
    public function _saveFields(CEvent $event)
    {
        ...
        // and save the responder model if no errors.
        if (empty($this->owner->errors)) {
            if (!$responder->save()) {
                $this->owner->errors[] = [
                    'show'      => true,
                    'message'   => $responder->shortErrors->getAllAsString(),
                ];
            }
        }
    }
    ...
    public function _setCorrectValidationRules(CModelEvent $event)
    {
        ...
        $min = $event->sender->field->minValue;
        $max = $event->sender->field->maxValue;

        $valueRule = ['value', 'numerical', 'integerOnly' => true, 'min' => $min, 'max' => $max];
        if ($event->sender->field->scenario === SurveyFieldNumber::SCENARIO_INTEGER_AND_FLOAT) {
            $valueRule = ['value', 'numerical', 'min' => $min, 'max' => $max];
        }
    ...
    protected function getValueModels(): array
    {
        ...
        $models = SurveyFieldNumber::model()->findAllByAttributes([
    ...
    protected function buildFieldArray(SurveyFieldValue $model): array
    {
        ...
        return hooks()->applyFilters('survey_field_builder_type_number_responder_build_field_array', [
