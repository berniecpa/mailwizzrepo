...
/**
 * SurveyFieldBuilderTypeTextResponder
 *
 ...
 */
class SurveyFieldBuilderTypeTextResponder extends SurveyFieldBuilderTypeResponder
{
    /**
     * @param CEvent $event
     *
    ...
    public function _setCorrectValidationRules(CModelEvent $event)
    {
        ...
        $min = (int)$event->sender->field->min_length;
        $max = (int)$event->sender->field->max_length;

        $rules->add(['value', 'length', 'min' => $min, 'max' => $max]);
    }
    ...
    protected function buildFieldArray(SurveyFieldValue $model): array
    {
        ...
        return hooks()->applyFilters('survey_field_builder_type_text_responder_build_field_array', [
