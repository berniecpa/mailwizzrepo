...
class SurveyFieldBuilderTypeYearsRangeResponder extends SurveyFieldBuilderTypeResponder
{
    ...
    protected function buildFieldArray(SurveyFieldValue $model): array
    {
        ...
        if (!$field->getVisibilityIsNone() || apps()->isAppName('customer')) {
            ...
            if ($fieldBehavior->getYearStart() > 0) {
                $yearStart = $fieldBehavior->getYearStart();
            }

            if ($fieldBehavior->getYearEnd() > 0) {
                $yearEnd = $fieldBehavior->getYearEnd();
            }
        ...
        return hooks()->applyFilters('survey_field_builder_type_yearsrange_responder_build_field_array', [
