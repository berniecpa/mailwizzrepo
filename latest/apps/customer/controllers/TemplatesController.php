...
class TemplatesController extends Controller
{
    ...
    public function actionGallery_import($template_uid)
    {
        ...
        /** @var CustomerEmailTemplate $newTemplate */
    ...
    public function actionCreate()
    {
        ...
        if (request()->getIsPostRequest() && ($attributes = request()->getPost($template->getModelName(), []))) {
    ...
    public function actionSave_screenshot($template_uid)
    {
        ...
        if (isset(app_param('POST')['data'])) {
            $data = ioFilter()->purify(app_param('POST')['data']);
        }
    ...
    public function actionTest($template_uid)
    {
        ...
        foreach ($emails as $email) {
            ...
            for ($i = 0; $i < 3; ++$i) {
                ...
                /** @var DeliveryServer $server */
                $server = DeliveryServer::pickServer((int)$server->server_id, $template, $dsParams);
