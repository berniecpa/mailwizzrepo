...
class BounceServer extends ActiveRecord
{
    ...
    public function getConnectionString(array $params = []): string
    {
        ...
        $params = hooks()->applyFilters('servers_imap_connection_string_search_replace_params', $params, $this);
        ...
        try {
            ...
            foreach ($results as $result) {
                ...
                if (isset($result['originalEmailHeadersArray'][$headerPrefixUp . 'CAMPAIGN-UID'], $result['originalEmailHeadersArray'][$headerPrefixUp . 'SUBSCRIBER-UID'])) {
                    $campaignUid   = trim((string)$result['originalEmailHeadersArray'][$headerPrefixUp . 'CAMPAIGN-UID']);
                    $subscriberUid = trim((string)$result['originalEmailHeadersArray'][$headerPrefixUp . 'SUBSCRIBER-UID']);
                }

                // 1.9.17
                if ((empty($campaignUid) || empty($subscriberUid)) && isset($result['originalEmailHeadersArray']['X-REPORT-ABUSE'])) {
                    $pattern = '/campaigns\/([a-z0-9]{13})\/report\-abuse\/([a-z0-9]{13})\/([a-z0-9]{13})/';
                    if ((bool)preg_match($pattern, $result['originalEmailHeadersArray']['X-REPORT-ABUSE'], $matches)) {
                        $campaignUid   = $matches[1];
                        $subscriberUid = $matches[3];
                    }
                }

                if ((empty($campaignUid) || empty($subscriberUid)) && isset($result['originalEmailHeadersArray']['LIST-UNSUBSCRIBE'])) {
                    $pattern = '/lists\/([a-z0-9]{13})\/unsubscribe\/([a-z0-9]{13})\/([a-z0-9]{13})/';
                    if ((bool)preg_match($pattern, $result['originalEmailHeadersArray']['LIST-UNSUBSCRIBE'], $matches)) {
                        $subscriberUid = $matches[2];
                        $campaignUid   = $matches[3];
                    }
                }
                // end 1.9.17 changes

                // start 1.9.24 changes
                if ((empty($campaignUid) || empty($subscriberUid)) && isset($result['originalEmailHeadersArray']['MESSAGE-ID'])) {
                    $messageId = trim((string)str_replace(['<', '>'], '', (string)$result['originalEmailHeadersArray']['MESSAGE-ID']));
                    $deliveryLog = CampaignDeliveryLog::model()->findByAttributes([
                        'email_message_id' => $messageId,
                    ]);
                    if (!empty($deliveryLog)) {
                        $subscriberUid = $deliveryLog->subscriber->subscriber_uid;
                        $campaignUid   = $deliveryLog->campaign->campaign_uid;
                    }
                }
                // end 1.9.24 changes

                if (empty($campaignUid) || empty($subscriberUid)) {
                    continue;
                }
                ...
                if (in_array($result['bounceType'], [BounceHandler::BOUNCE_SOFT, BounceHandler::BOUNCE_HARD])) {
                    ...
                    // 1.4.4
                    if ($logger) {
                        call_user_func($logger, sprintf('Subscriber uid: %s is %s bounced with the message: %s.', $subscriberUid, (string)$result['bounceType'], 'FBL complaint!'));
                    }
